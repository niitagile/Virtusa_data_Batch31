...
public class DistributorItemDAO {
	...
	public int deleteItem(String distributor) {
		sql="DELETE FROM distributor_item WHERE distributor=?";
		try {
			
			row=this.jdbcTemplate.update(sql,distributor);
		}
		catch(Exception e) {
			e.printStackTrace();
			row=0;
	...
	public int getItemIdbyId(int id) {
		sql="SELECT * FROM distributor_item WHERE id=?";
		//List<DistributorItemBean> itemsId=new ArrayList();
	...
	public void updateItemAfterOrder(int itemsId,String itemName,int quantity) {
		...
		try {
			...
			row= this.jdbcTemplate.update(sql,itemsId,itemName,quantity);
		}
		catch(Exception e) {
			e.printStackTrace();
		}
